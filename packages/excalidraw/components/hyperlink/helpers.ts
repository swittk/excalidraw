import { pointFrom, pointRotateRads } from "ex-excalidraw-math";

import { MIME_TYPES } from "ex-excalidraw-common";
import { getElementAbsoluteCoords } from "ex-excalidraw-element";
import { hitElementBoundingBox } from "ex-excalidraw-element";

import type { GlobalPoint, Radians } from "ex-excalidraw-math";
...
const ELEMENT_LINK_SVG = `<svg  xmlns="http://www.w3.org/2000/svg"  width="16"  height="16"  viewBox="0 0 24 24"  fill="none"  stroke="#1971c2"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-arrow-big-right-line"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 9v-3.586a1 1 0 0 1 1.707 -.707l6.586 6.586a1 1 0 0 1 0 1.414l-6.586 6.586a1 1 0 0 1 -1.707 -.707v-3.586h-6v-6h6z" /><path d="M3 9v6" /></svg>`;

const createLinkImage = (svg: string) => {
  if (typeof document === "undefined") {
    return null;
  }

  const image = document.createElement("img");
  image.src = `data:${MIME_TYPES.svg}, ${encodeURIComponent(svg)}`;
  return image;
};

let externalLinkImage: HTMLImageElement | null = null;
let elementLinkImage: HTMLImageElement | null = null;

export const getExternalLinkImage = () => {
  if (!externalLinkImage) {
    externalLinkImage = createLinkImage(EXTERNAL_LINK_SVG);
  }
  return externalLinkImage;
};

export const getElementLinkImage = () => {
  if (!elementLinkImage) {
    elementLinkImage = createLinkImage(ELEMENT_LINK_SVG);
  }
  return elementLinkImage;
};
